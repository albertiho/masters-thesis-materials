...
from research.training.scripts.validate_iforest_mh5_smoke import build_quick_subset, run_smoke_validation
...
def test_build_quick_subset_limits_products_and_history() -> None:
    train_df = _price_frame(n_products=10, observations_per_product=12, start="2026-01-01")
    test_df = _price_frame(n_products=10, observations_per_product=2, start="2026-02-01")
    train_df["product_id"] = train_df["product_id"].str.extract(r"(\d+)$")[0].astype(int)
    test_df["product_id"] = test_df["product_id"].str.extract(r"(\d+)$")[0].astype(int)

    train_subset, test_subset = build_quick_subset(
        train_df,
        test_df,
        max_products=4,
        max_history_per_product=5,
        min_test_rows=6,
        max_test_rows=8,
    )

    assert train_subset["product_id"].nunique() <= 4
    assert test_subset["product_id"].nunique() <= 4
    assert len(test_subset) >= 6
    assert train_subset.groupby("product_id").size().max() <= 5


def test_run_smoke_validation_loads_model_and_runs_fast_tuning(tmp_path: Path) -> None:
    ...
    persistence.save_isolation_forest(detector, "COMPETITOR_1_COUNTRY_1_mh5", len(X_train))

    output_csv = tmp_path / "results" / "smoke.csv"
