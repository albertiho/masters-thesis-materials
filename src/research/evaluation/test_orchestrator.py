...
class TestOrchestrator:
    ...
    def _process_all_rows(
        ...
        For ML detectors (Autoencoder, DepAD, Isolation Forest), uses batch
    ...
    def _process_all_rows_with_obs(
        ...
        def process_evaluator_sequential(evaluator: DetectorEvaluator) -> tuple[str, list[AnomalyResult]]:
            ...
            print(f"  [Sequential] {evaluator.name} done in {elapsed:.1f}s ({n_rows / safe_elapsed:.0f} rows/s)")
    ...
    def _compute_per_type_detection(
        ...
        if df_injected is not None and "__injected_anomaly_type__" in df_injected.columns:
            # Build type counts from DataFrame columns (aligned after sorting)
            type_counts: dict[str, dict[str, int]] = {}
            
            for i in range(len(predictions)):
                anomaly_type = df_injected.iloc[i]["__injected_anomaly_type__"]
...
def create_expanded_statistical_evaluators(
    *,
    ...
    """Create evaluators for the expanded statistical detector roster.

    This helper is intended for local experimentation and smoke testing of the
    evaluation stack. It does not modify the published manifest-driven detector
    comparison surfaces.
    """
