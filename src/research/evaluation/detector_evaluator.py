...
class DetectorEvaluator:
    ...
    def __init__(
        ...
        # The standalone thesis repo uses a local-only TemporalCacheManager.
    ...
    def process_row(
        ...
        # Get price history for IQR/ensemble detectors
    ...
    def _get_price_history(
        ...
        """Return raw cached price history for history-dependent detectors."""
        cache_entry = self.temporal_cache.get(product_id, competitor_id)
        if cache_entry is None or len(cache_entry.price_history) == 0:
            return None
        return cache_entry.price_history.tolist()
    ...
    def _detect(
        ...
        Args:
            ...
            price_history: Optional price history (for IQR/ensemble).
        ...
        if self._detector_type == "combined":
        ...
        elif self._detector_type == "iqr":
            # IQR detector needs price_history
            return self.detector.detect(
                numeric_features=numeric_features,
                temporal_features=temporal_features,
                price_history=price_history,
            ...
            # Custom history-dependent detectors should receive raw price history
            return self.detector.detect(
                numeric_features=numeric_features,
                temporal_features=temporal_features,
                price_history=price_history,
        ...
        elif self._detector_type in ("isolation_forest", "autoencoder", "depad"):
    ...
    def _numeric_features_from_tuple(
        ...
        if "country" in col_map:
            country_val = row[col_map["country"]]
            if pd.notna(country_val):
                record_country = str(country_val)
    ...
    def _detect_detector_type(self, detector: Any) -> str:
        ...
        Returns:
            Detector type string: "combined", "statistical_ensemble", "iqr", "sanity",
            "isolation_forest", "autoencoder", "depad", or "statistical".
        """
    ...
    def _process_batch_combined(
        ...
        while any(queue for queue in product_queues.values()):
            ...
            for row_ctx in round_rows:
                ...
                # Get price history for IQR/ensemble detectors within combined
