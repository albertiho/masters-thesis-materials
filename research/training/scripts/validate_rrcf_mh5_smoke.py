#!/usr/bin/env python3
"""Quick smoke validation for trained mh5 RRCF models.

The script mirrors the IF/EIF smoke-validation flow: it loads existing mh5
RRCF models, picks a small deterministic subset of one or more train/test
pairs, and runs a fast threshold-tuning pass to validate that the saved models
can be loaded, scored, and tuned on real data. Saved models are left untouched
unless ``--write-threshold`` is provided.
"""
...
def update_model_threshold(
    ...
    """Update the anomaly_threshold in a saved local RRCF model."""
    from joblib import dump as joblib_dump
    from joblib import load as joblib_load

    model_path = persistence._get_model_path(model_name, "rrcf", "model.joblib")
...
def run_smoke_validation(
    *,
    ...
    """Run a quick RRCF smoke-validation/tuning pass on real mh5 models."""
    ...
    for candidate in selected_candidates:
        ...
        if not persistence.model_exists(candidate.model_name, "rrcf"):
            results.append(
                SmokeValidationResult(
                    ...
                    message="RRCF model not found in local persistence",
                )
            )
            continue

        try:
            detector = persistence.load_rrcf(candidate.model_name)
...
def _default_output_csv() -> str:
    ...
    return str(
        Path("results") / "validation" / f"rrcf_mh5_smoke_tuning_{timestamp}.csv"
    )


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Quick smoke validation for trained mh5 RRCF models."
    )
    ...
    print("=" * 70)
    print("RRCF mh5 Smoke Validation")
    print("=" * 70)
