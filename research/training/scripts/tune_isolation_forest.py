#!/usr/bin/env python3
"""Tune Isolation Forest thresholds using held-out test data.

This script finds optimal anomaly_threshold values for trained Isolation Forest
models by evaluating them on test data with synthetic anomaly injection.
...
8. Updates saved local model files (unless --dry-run)

Usage:
    # Tune all models using test data (known products scenario)
    python scripts/tune_isolation_forest.py --file-suffix "_test"

    # Cold-start scenario (no price history)
    python scripts/tune_isolation_forest.py --file-suffix "_test" --cold-start

    # Dry run (show results without updating saved models)
    python scripts/tune_isolation_forest.py --file-suffix "_test" --dry-run

    # Custom threshold range
    python scripts/tune_isolation_forest.py --file-suffix "_test" --min-threshold 0.3 --max-threshold 0.9

Prerequisites:
    Models must be trained locally first:
    python scripts/train_isolation_forest.py --granularity both --file-suffix "_train"
"""
...
def update_model_threshold(
    ...
    """Update the anomaly_threshold in a saved local model.
    ...
    # Download model
    model_path = persistence._get_model_path(model_name, "isolation_forest", "model.joblib")
    ...
    old_threshold = saved_data["config"].get("anomaly_threshold", "unknown")
    ...
    logger.info(f"  Updated threshold: {old_threshold:.4f} -> {new_threshold:.4f}")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Tune Isolation Forest thresholds")
    parser.add_argument(
        "--data-path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.3,
        help="Minimum threshold to test (default: 0.3)",
    )
    parser.add_argument(
        "--max-threshold",
        type=float,
    ...
    print("=" * 70)
    print("Isolation Forest Threshold Tuning (Parallel Trials)")
    print("=" * 70)
    print(f"Data path: {args.data_path}")
    print(f"File suffix: {args.file_suffix}")
    print(f"Granularity: {args.granularity}")
    print(f"Threshold range: {args.min_threshold} - {args.max_threshold} ({args.steps} steps)")
    ...
    for granularity in granularities:
        ...
        if not files:
            print(
                f"No test files found for granularity {granularity!r} under "
                f"{args.data_path} with suffix {args.file_suffix!r}"
            )
            continue
        ...
        for filepath in files:
            ...
            if not persistence.model_exists(model_name, "isolation_forest"):
                print("    [SKIP] Model not found locally")
                skipped_no_model.append(model_name)
                continue
            ...
            # Use linear spacing for normalized IF scores (0-1 range)
            ...
            result = run_tuning_trials(
                ...
                # IF uses drop_range (0.1, 0.5) - original script's values
                drop_range=(0.1, 0.5),
            )
            ...
            # Print results
            print(f"    Current: threshold={result.current_threshold:.4f}, F1={result.current_f1:.1%}")
            print(f"    Best:    threshold={result.best_threshold:.4f}, F1={result.best_f1:.1%}")
            ...
            if args.verbose:
                ...
                for r in result.all_results:
                    marker = " *" if abs(r["threshold"] - result.best_threshold) < 0.001 else ""
                    print(f"    {r['threshold']:>10.4f} {r['precision']:>9.1%} {r['recall']:>9.1%} {r['f1']:>9.1%}{marker}")
            ...
            if not args.dry_run and should_update:
                try:
                    update_model_threshold(persistence, model_name, result.best_threshold)
    ...
    if skipped_no_model:
        print(f"\nModels not found locally ({len(skipped_no_model)}):")
        for m in skipped_no_model:
            print(f"  - {m}")
        print("\nTrain models first with:")
        print('  python scripts/train_isolation_forest.py --granularity both --file-suffix "_train"')

    if all_results:
        print(f"\n{'Model':<30} {'Current':>10} {'Best':>10} {'F1':>8} {'Improvement':>12}")
        print("-" * 70)
        for r in all_results:
            imp_str = f"+{r.improvement_pct:.1f}%" if r.improvement_pct > 0 else f"{r.improvement_pct:.1f}%"
            print(
                f"{r.model_name:<30} {r.current_threshold:>10.4f} {r.best_threshold:>10.4f} "
